...
Read `skills/babysit-pr/references/heuristics.md` for a concise checklist.
...
It intentionally surfaces approved reviewer bot feedback (for example comments/reviews from `chatgpt-codex-connector[bot]`, GitHub Copilot, or Devin) in addition to human reviewer feedback. Most unrelated bot noise should still be ignored.
For safety, the watcher only auto-surfaces trusted human review authors (for example repo OWNER/MEMBER/COLLABORATOR, plus the authenticated operator) and approved review bots (currently Codex, Copilot, and Devin).
...
- Heuristics and decision tree: `skills/babysit-pr/references/heuristics.md`
- GitHub CLI/API details used by the watcher: `skills/babysit-pr/references/github-api-notes.md`
